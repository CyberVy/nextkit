...
export function register_web_message_rpc(){
    handle_request("rpc_execute", (data) => {
        const args = data.args
        if (Array.isArray(args)){
            execute(...args as [string, unknown[], string | undefined])
        }
    })
    handle_request("rpc_assign", (data) => {
        const args = data.args
        if (Array.isArray(args)){
...
export function notify_rpc_success_to_window(_window: Window){
    request_to_window(_window, { type: "register_web_message_rpc", result: "success", origin: location.href }).catch(err => console.warn(err))
}
